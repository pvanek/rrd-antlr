...
internal class LexicalRule(ruleName: String, alignment: Alignment = Alignment.MIDDLE) :
    AbsDiagramNode(alignment, ruleName, DiagramSettings.STYLE_LEXER) {

    override fun calcWidth(): Float {
        return java.lang.Float.max(
            (caption.length * DiagramSettings.ruleCharSize),
            DiagramSettings.itemHeight
        ) + 2 * DiagramSettings.innerPadding
    }
        ...
        append(
            svgAnchor(
                ...
                svgRectangle(startRect, y, rectWidth, DiagramSettings.itemHeight, styleClass) +
                        svgText(
                            startRect + (rectWidth / 2),
                            y + (DiagramSettings.itemHeight / 2),
                            caption,
                            styleClass
                        )
            )
